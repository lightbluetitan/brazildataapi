...
#' Get Brazil's GDP (current US$) from World Bank
#'
#' @description
#' Retrieves Brazil's Gross Domestic Product (GDP) in current US dollars
...
#'
#' @export
get_brazil_gdp <- function() {
  url <- "https://api.worldbank.org/v2/country/BRA/indicator/NY.GDP.MKTP.CD?format=json&date=2010:2022&per_page=100"
  res <- httr::GET(url)
