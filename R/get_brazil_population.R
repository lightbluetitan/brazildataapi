...
#' Get Brazil's Total Population from World Bank
#'
#' @description
#' Retrieves Brazil's total population for the years 2010 to 2022
...
#'   \item \code{country}: Country name ("Brazil")
#'   \item \code{year}: Year of the data (integer)
#'   \item \code{value}: Population as a numeric value
#'   \item \code{value_label}: Formatted population with commas (e.g., "210,000,000")
#' }
#'
#' @details
...
#'
#' @export
get_brazil_population <- function() {
  url <- "https://api.worldbank.org/v2/country/BRA/indicator/SP.POP.TOTL?format=json&date=2010:2022&per_page=100"
  res <- httr::GET(url)
