...
#'
#' This function lists all datasets available in the 'BrazilDataAPI' package.
#' If the 'BrazilDataAPI' package is not loaded, it stops and shows an error message.
...
#' }
#' @export
view_datasets_BrazilDataAPI <- function() {
  # Check if the package is loaded
  if (!"BrazilDataAPI" %in% .packages()) {
    stop("The 'BrazilDataAPI' package must be loaded to view its datasets.")
  }

  # Extract dataset information
  datasets_info <- utils::data(package = "BrazilDataAPI")$results

  # Check if there are datasets available
  if (nrow(datasets_info) == 0) {
    message("No datasets are currently available in the 'BrazilDataAPI' package.")
    return(character(0))
  }
  ...
  message("Datasets available in the 'BrazilDataAPI' package:")
